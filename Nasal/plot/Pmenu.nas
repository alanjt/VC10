...
#   })();
   ...
   for( var slot = 0 ; slot < 6 ; slot += 1) { # Slots 0-5
      ...
      me._Ofact[slot].Settext(sprintf("%8.2e",a)); # Should change in e-factor ?
...
Menu.ErDel = func (l) {
   if(me._Caller.Lexist(l) == 1 ){
      ...
      me._Oprop[l].Settext("");
      me._Otxt[l].SetColor(2);
      me._Ofact[l].Settext(sprintf("%8.2e",1)); # Should change in e-factor ?
      me._Oftxt[l].SetColor(2);
      me._Oena[l].SetText("Enable");
...
Menu.TextIn = func (but,l) {
   ...
   me._Ofact[l].Settext(sprintf("%8.2e",ONumber));
   var Number = 0;
...
Menu.OpenChsr = func (x) {
   gui.popupTip(sprintf("Menu.OpenChsr for line %d",x),2);
...
Menu.Marker = func () {
   ...
   #gui.popupTip(sprintf("Menu.Marker Placeholder for TickMarker." ),2);
   return me;
};
...
#   #gui.popupTip(sprintf("Menu.OpenChsr Placeholder for Update speed set." ),2);
#   return me;
##};
