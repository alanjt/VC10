...
# use: Load.new(parent) parent needed to call-back
Load.new = func (x,cnt) {
   var me = { parents : [Load] ,
      _Caller: 0, # need to reset _Load, double menu prevention.
      _Cl: 0,     # Curent line
      _Width: 0,
      _Height: 0,
      _Dialog: 0,
      ...
      _Otxt: [0],               # Input text/Selected text
      _Helper: 0,               # handle to helper functions
   };
   me._Caller = x;
   me._Cnt = cnt;
   printf("Load.new called with number %d",me._Cnt);
   return me;
}
...
#   var Files = directory(FgHome~"/");  # Just $FGHOME
   ...
   me._Cnt = size(me._ToRead);  # Double use of this var !
   var i = 0;
...
# Scroll the contend # Stupit hard coding...
Load.Scroll = func (y) {
...
Load.Choice = func (y,b,Y) {
   if((Y != 0) or (b != 0))return;
   var i = int((y-60)/20);
...
#Load properties into proptree and into this program
Load.Go = func () {
   printf("Props will be loaded from %s",me._Otxt[0].Gettext());
   for( var i = 0 ; i < 6 ; i += 1) {
      setprop("/sim/fgplot2/Lns["~i~"]","");
      setprop("/sim/fgplot2/Color["~i~"]",0);
      setprop("/sim/fgplot2/Factor["~i~"]",-1);
      setprop("/sim/fgplot2/Width["~i~"]",1);
   };
   var FgHome = getprop("/sim/fg-home");
   var f = me._Otxt[0].Gettext();
   var filename = substr(f,0,size(f)-4);
   io.read_properties(FgHome~"/state/"~filename,"/sim/fgplot2" ); 
#   io.read_properties(FgHome~"/"~filename,"/sim/fgplot2" ); 
   for( var i = 0 ; i < 6 ; i += 1) {
      me._Caller._Lns[i].SetProperty(getprop("/sim/fgplot2/Lns["~i~"]"));
      me._Caller._Lns[i].SetColor(getprop("/sim/fgplot2/Color["~i~"]"));
      me._Caller._Lns[i].SetFactor(getprop("/sim/fgplot2/Factor["~i~"]"));
      me._Caller._Lns[i].SetWidth(getprop("/sim/fgplot2/Width["~i~"]"));
   };
   #After this Loadwindow is gone, The caller should be repainted. !
   # Now the contend of the menu is invalid.
   me._Caller._Menu.RePopulate();
   me.Del(1);
   return me;
};

# Called TextIn, but does only numbers in !
Load.TextIn = func (but,x,y) {
   var Number = 0;
   if((but == 0) and (x == 0) and (y == 0)){ # input when left mouse and no drag !?
   me._Otxt[0].SetColor(1);
   print("Load.TextIn active.");
      if(me._Listener[0] ==  0){
...
# Signal Caller menu is gone and del.
Load.Del = func (bt) {
